"""Shared startup environment validation module.
...
Each service should call startup_check() at startup.

Moved from trader.config to the shared prism-schemas package so that
both trader and sentinel can import it without creating a cross-service
dependency.
"""
...
logger = structlog.get_logger("prism.startup")
...
def check_geofence(locale: str | None = None) -> bool:
    ...
    - Live mode (PRISM_TRADE_MODE=live): hard failure (sys.exit(1))
      because jurisdiction must be confirmed before real trades.
    - Paper mode (default): warning logged, returns True since
      paper trades carry no regulatory risk.
    """
    locale = locale or os.environ.get("LOCALE", "")
    if not locale:
        trade_mode = os.environ.get("PRISM_TRADE_MODE", "paper").strip().lower()
        if trade_mode == "live":
            logger.error(
                "geofence_check_failed",
                reason="LOCALE not set in live mode — jurisdiction must be confirmed",
                trade_mode=trade_mode,
            )
            print(
                "FATAL: LOCALE env var is required in live trade mode. "
                "Set LOCALE to your ISO-3166-1 alpha-2 country code to confirm jurisdiction.",
                file=sys.stderr,
            )
            sys.exit(1)
        # Paper mode — safe to proceed without locale
        logger.warning("geofence_check_skipped", reason="LOCALE not set (paper mode)")
        return True
