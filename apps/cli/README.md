...
`prism demo` is safe by default: it fetches a public report, obtains an x402 quote, writes JSON + Markdown receipts under `.prism/receipts/`, and prints the paid command. It only signs/submits payment when `--pay` is explicitly passed.
