...
async function writeAuditEvent(params: {
  ...
  try {
    ...
    // Swallow the error — the operator will notice when audit events go missing.
  }
}

/**
 * POST /api/admin/schedule/start
 *
 * Proxies the trader's `POST /schedule` endpoint.  Requires a valid
 * OPERATOR_ADMIN_TOKEN.  Writes an audit event to operator_events on every
 * attempt — success, failure, or unauthorized.
 *
 * This route does NOT accept or forward a trade_mode parameter
 * (VAL-ADMIN-011).  The operator token is NEVER forwarded to the trader
 * (VAL-ADMIN-012).  The POST body may be empty (VAL-ADMIN-013).
 */
export async function POST(request: NextRequest): Promise<NextResponse> {
  // --- Auth gate ---
  if (!isOperatorAdminRequest(request)) {
    // Write unauthorized audit event (actor = "unknown", no token value)
